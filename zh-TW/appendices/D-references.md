...
*第一部的規範性依據。本指南所有 Verilog 的語法、語意與 simulation 行為，均依此標準定義。*
...
涵蓋 SVA 在驗證環境中的整合方式，可補充第三部的 assertion methodology。

**《SystemVerilog Assertions 应用指南》**
*中文實務 SVA 參考書，涵蓋各種 assertion 編碼樣式。對偏好以中文為主要資料來源的讀者，可搭配第三部一起使用。*
...
*以可驗證性為核心的 RTL 撰寫方式的奠基著作。書中介紹的設計意圖（design intent）原則，正是第三部所採用的 Assertion-Based Verification（ABV）方法論的思想基礎。*
...
第 3 版。Springer, 2012。
*涵蓋受約束隨機驗證（constrained-random verification）、coverage 驅動驗證以及 SystemVerilog 的類別型物件導向功能。本指南刻意省略語言的驗證面向；對需要撰寫 testbench 的設計者而言，Spear & Tumbush 是推薦的下一步讀物。*
...
- 各工具廠商（Synopsys、Cadence、Siemens EDA、Aldec）均發布針對其 simulator 與 formal engine 的應用注釋；當標準語意與工具特定行為有差異時，應查閱這些文件。
