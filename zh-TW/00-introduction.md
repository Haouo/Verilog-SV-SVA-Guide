...
- 理解為何 assertion 是設計者的職責，而不只是驗證工程師的工作。
- 認識三個部分如何銜接，以及該如何閱讀。
...
並交由合成工具轉成邏輯閘的人。它假設你已了解數位邏輯——組合與循序電路、clock、
reset，以及基本的合成流程；不會從零教起邏輯設計。它教的是設計者用來描述邏輯的語言，
以及設計者用來陳述「邏輯該做什麼」的 assertion language。
...
*設計意圖（design intent）*。它們存在你的腦中、規格書裡，或一句註解中——這些都是
工具無法檢查的地方。

assertion 把設計意圖搬進程式碼，化為 simulator 或 formal tool 每個週期都能檢查的形式。
當設計日後違反了這個意圖——無論在你自己的 simulation、同事的 simulation，或區塊層級的回歸測試中
——assertion 會在失效的當下、當地觸發，而不是等到三個模組之外、症狀才浮現的地方。

這正是 SVA 屬於設計者的原因。你才是知道意圖的人。在撰寫 RTL 的同時把意圖寫下來，
能在記憶最清晰時將它捕捉，並轉為永久、可被機器檢查的契約。
...
設計者需要辨識它們時提及——例如，知道在 formal flow 中 `assume` 該放在哪裡。唯一深入
