...
- 理解為何斷言（assertion）是設計者的職責，而不只是驗證工程師的工作。
- 認識三個部分如何銜接，以及該如何閱讀。

## 本指南的讀者對象

本指南寫給數位設計工程師：撰寫 RTL（Register-Transfer Level，暫存器轉移層級）程式碼、
並交由合成工具轉成邏輯閘的人。它假設你已了解數位邏輯——組合與循序電路、時脈、
重置，以及基本的合成流程；不會從零教起邏輯設計。它教的是設計者用來描述邏輯的語言，
以及設計者用來陳述「邏輯該做什麼」的斷言語言。
...
斷言把設計意圖搬進程式碼，化為模擬器或形式化（formal）工具每個週期都能檢查的形式。
當設計日後違反了這個意圖——無論在你自己的模擬、同事的模擬，或區塊層級的回歸測試中
——斷言會在失效的當下、當地觸發，而不是等到三個模組之外、症狀才浮現的地方。

這正是 SVA 屬於設計者的原因。你才是知道意圖的人。在撰寫 RTL 的同時把意圖寫下來，
能在記憶最清晰時將它捕捉，並轉為永久、可被機器檢查的契約。

> **設計意圖。** RTL 描述硬體*如何*行為；斷言描述對該行為而言*什麼必須為真*。
> 兩者並用遠比單獨使用任一者更強：RTL 可能出錯，而斷言會指出這個錯誤。

## 本指南不涵蓋的內容

這是一本設計指南，不是驗證課程。建構測試平台（testbench）、UVM、基於類別的隨機化
（randomization），以及覆蓋率（coverage）方法論都是各自有完整文獻的大主題。我們只在
設計者需要辨識它們時提及——例如，知道在形式化流程中 `assume` 該放在哪裡。唯一深入
探討的驗證相關主題是 SVA，因為它就是設計意圖。

## 如何閱讀本指南

若這些語言對你是全新的，請依序閱讀。第一部建立 Verilog 基礎，第二部加入 SystemVerilog
的設計功能，第三部則運用兩者以斷言表達意圖。
