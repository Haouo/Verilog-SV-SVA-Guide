...
注意，套件常數無法像模組參數那樣被覆寫——套件不會被例化，因此套件中宣告的 `parameter` 實質上是固定常數。請優先使用 `localparam`，明確表達「永不覆寫」的意圖。
...
- **套件中 `parameter` 與 `localparam` 的混淆。** 兩者在套件中實質上都是固定常數——套件不會被例化。請優先使用 `localparam`，明確表達永不覆寫的意圖。
