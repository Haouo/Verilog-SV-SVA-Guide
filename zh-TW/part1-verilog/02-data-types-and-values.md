...
Verilog 的 type 與 value 主要是在回答一個問題：這個 bit pattern 是由哪種 object drive 或保存。
`wire` 是由 driver resolve 出來的 connection；`reg` 是 procedural variable，保存最後一次 assigned
value。這兩個字本身不保證「實體導線」或「flip-flop」，因此必須把 type 和 assignment
方式一起讀。
...
很多 Verilog bug 不是來自冷門 syntax，而是 value 比設計者預期的多一 bit、少一 bit、
帶 signedness，或含有 unknown。

## 線網與變數

Verilog 有兩類資料物件，這種區分常令初學者困惑，因為它與軟體直覺不符。

- **線網（net）**（最常見的型別是 `wire`）model 實體連接。它不儲存數值，而是反映驅動它的訊號。線網必須被持續驅動——由模組輸出、基本邏輯元件，或 `assign` 語句。
...
- 四種狀態 `0 1 x z` model 真實硬體；非預期的 `x` 意味著問題。
