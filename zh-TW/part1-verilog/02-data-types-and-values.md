...
Verilog 的型別與數值，多半是在回答一個問題：這組位元樣式是由哪一種物件來驅動或保存。
`wire` 是從各個驅動源解析出來的連接，`reg` 則是一個程序變數，保存最後一次被指定的值。
這兩個字本身都不保證對應到「實體導線」或「flip-flop」，所以一定要把型別和它的指定
方式放在一起看。

第二個心智模型是位元寬度。硬體沒有抽象的整數，只有固定數量的位元。每一條字面值、向量、
串接與位元延伸規則，都是在決定有多少位元，以及 `x` 和 `z` 會如何傳播。許多 Verilog 的錯誤
都不是來自冷門語法，而是某個值比設計者以為的多了一位、少了一位、帶了正負號，或含有
未知值。
...
- **線網（net）**（最常見的型別是 `wire`）對應實體連接。它不儲存數值，而是反映驅動它的訊號。線網必須持續被驅動，來源可以是模組輸出、基本邏輯元件，或 `assign` 語句。
...
- `z` 表示沒有任何東西驅動該線網，例如三態（tri-state）匯流排，或浮接的輸入。
- `x` 表示數值未知，例如未初始化的暫存器、多重驅動衝突，或讀取未定義值的結果。

在 simulation 中，`x` 常指示真實的錯誤：從未被重置的暫存器，或競態（race）。遇到非預期的 `x`，應追蹤其根源，而非加以遮蔽。斷言（assertion，第三部）是在不應出現 `x` 之處捕捉它的有效手段。
...
- **忽略 `x`。** 非預期的 `x` 通常代表遺漏重置或競態，而非無關緊要的外觀問題。請追蹤它。

## 小結

- 線網（`wire`）攜帶被驅動的值；變數（`reg`）持有被指定的值。
- 向量使用 `[msb:lsb]`，慣例上 LSB 為 0。
- 四種狀態 `0 1 x z` 對應真實硬體；非預期的 `x` 代表出了問題。
