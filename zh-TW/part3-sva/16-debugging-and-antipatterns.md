...
看到 assertion fail 時，最直覺的反應是「RTL 壞了」。但實務上還有另一半機率：property 寫錯 edge、reset 沒關、antecedent 太寬、或 consequent 的期限抓錯。若不先釐清是哪一種，debug 會變成在波形裡追一條錯誤規格。

所以除錯流程要從故事線開始：這次 attempt 何時被開啟？它應該在哪個週期履行義務？失敗時看到的值是 sampled value 還是當前值？本章用這些問題串起常見反樣式，讓失敗訊息變成可追的線索。
