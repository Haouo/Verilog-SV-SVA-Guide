...
想像一份回歸報告顯示所有 assertion 都通過，但測試其實從未發出 `req`。`req |=> gnt` 當然不會失敗，因為它從未真正要求 `gnt` 出現。這種「綠燈但沒測到」比紅燈更危險，因為它會讓人誤以為設計已被驗證。

coverage 與 vacuity 要補的是這個信任缺口。`assert` 告訴你規則沒有被違反，`cover` 告訴你規則的觸發情境真的發生過。本章把這兩者配成一套收斂紀律：不只要沒有 fail，還要知道每條重要規則確實被執行。
