...
大多數設計規則都有明確期限：一到四拍內回應、保持三拍、下一拍不能再高。這些用前面章節的 delay、repeat 或 until 就夠了。但有些規則長得像「只要還沒結束，就繼續套用同一條規則」，例如 lock 後永遠保持 busy，或某個條件每拍都要把義務延到下一拍。

recursive property 只在這種自相似規則下才值得出場。它不是讓 property 變聰明的技巧，而是把「現在成立，而且下一拍又回到同一個問題」寫成有限文字。本章會先示範這個展開方式，再提醒何時該避免遞迴。
