...
先拿一句自然語言來看：「`req` 發生後，下一拍要看到 `gnt`。」這句話裡有兩個部分：`req` 是起點，`gnt` 不是同一拍，而是在下一個 clock。單一布林只能說「這一拍為真」，無法表達「下一拍也要發生另一件事」。

sequence 就是為了填這個空缺。`req ##1 gnt` 把時間線切成兩個取樣點：起點檢查 `req`，下一點檢查 `gnt`。本章後面的固定延遲、範圍延遲與重複，都是在回答同一個問題：這個行為形狀跨了哪些 clock，哪些週期必須被檢查，哪些只是允許經過。
