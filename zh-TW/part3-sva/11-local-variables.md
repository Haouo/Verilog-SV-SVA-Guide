...
- 避免使區域變數令人意外的 sampling 與流動陷阱。

## 設計者 mental model

local variable 讓 assertion 記住某一次 attempt 關聯的 value。這是 data-carrying intent 的關鍵：
...
指定也可在 sequence 推進時更新區域變數——例如在重複（repetition）的每次迭代上遞增一個計數器。沿著匹配執行緒（thread）前傳的值，永遠是最近一次的指定。

## sampling：區域變數使用 sampled value

區域變數的指定捕捉其右側的 **sampled value**——Preponed 區域中的值，即 assertion 其餘部分所見的同一個值（第 2 章）。它不會捕捉週期中途或帶毛刺的值。正是這一點，使所捕捉的資料能與設計自身的 sampling 逐週期對齊。
...
由於捕捉的是 sampled value，稍後將它與另一個 sampled value 比較，便是同類對同類的比較——這正是管線化檢查所需的語意。
...
> **設計意圖。** 區域變數讓 assertion 能說「*這個*輸出必須匹配*那個*輸入」——把回應 bind 到觸發它的那個特定值，而不只是 bind 到「發生了某個回應」。這是設計者陳述資料路徑契約的方式：進去的數，就是（經適當轉換後）出來的數，每一次，穿過每一級。
...
在一條從未指定該變數的執行緒上讀取區域變數，對該執行緒而言其值未定義，使檢查失去意義。請務必在每條稍後會讀取它的路徑上指定區域變數——通常在 antecedent 的觸發處，使每條存活的執行緒都帶有已定義的值。

## 常見陷阱

- **指定前讀取。** 在當前執行緒上未指定的區域變數沒有有意義的值。請在觸發處指定它，使每條存活執行緒都有它。
- **期望單一共用變數。** 每次嘗試各有自己的副本；你無法用區域變數在*不同*嘗試*之間*傳遞狀態。那需要用設計狀態或另一套機制。
- **忘記值是 sampling 得來的。** 所捕捉的值是 Preponed sampling，而非即時或帶毛刺的值。這對逐週期對齊的檢查是正確的，卻會令期望阻塞式指定時序的人感到意外。
- **在不匹配的分支上指定。** 在失敗分支上指定的值不會向前流動。請把指定放在必須承載它的執行緒上。
- **把 antecedent 弄得過於複雜。** 在單一 assertion 中做深度的區域變數記帳，難以閱讀與除錯。請偏好能表達該資料 bind 的最簡捕捉。

## 小結

- 區域變數是某一次 assertion 嘗試所私有的儲存空間，在匹配時寫入、稍後讀取。
- 以 `(boolean, var = expr)` 形式指定；所捕捉的是 sampling（Preponed）值。
