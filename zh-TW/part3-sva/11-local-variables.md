...
有些規則不只要求「有回應」，還要求「回應對得上當初那個請求」。例如讀回來的 address tag 要等於發出 request 時的 tag，或資料回傳要等於幾拍前寫入時取樣到的 payload。若只看當前訊號，你會把後來已經改掉的值拿來比較，檢查就失真了。

local variable 的角色，就是替每一次 assertion attempt 留下一份私有筆記。它在觸發點記住值，到了 consequent 再拿出來比。這章的重點不是把它想成 RTL 暫存器，而是把它想成「這一次規則檢查」自己的上下文。
