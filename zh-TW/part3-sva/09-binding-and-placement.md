...
寫出一條 assertion 之後，下一個問題通常不是語法，而是放在哪裡。若檢查的是模組內部的不變式，放在 RTL 旁邊最清楚；若檢查的是第三方 IP、generated RTL，或驗證團隊不該改動的檔案，直接塞進原始碼反而會破壞 ownership。

因此放置策略是在回答兩件事：誰擁有這條規則，以及它需要看見哪些訊號。inline、separate module、interface、checker 與 `bind` 都是在這兩個軸上取捨。本章先從這個權責問題出發，再介紹各種放置方式。
