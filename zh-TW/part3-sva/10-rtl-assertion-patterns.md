...
[← 綁定與放置](09-binding-and-placement.md) · [目錄](../README.md) · [下一章：區域變數 →](11-local-variables.md)

## 學習目標

- 辨識真實 RTL 檢查反覆出現的形態。
- 為 handshake、stable-until-accept 與仲裁撰寫斷言。
- 將 one-hot 與 gray-code 不變式直接編碼為性質。
- 陳述 FIFO 的安全性：不溢位、不下溢、空時不讀。
- 以斷言防止資料出現 `X`，並了解 SVA 在 CDC 檢查上的侷限。

本章是一座樣式庫。每個樣式將一段實作好的 SVA 片段，與它所捕捉的設計意圖配對，讓你能將它搬進真實區塊並調整訊號名稱。所有片段都假設範圍內已有 `default clocking @(posedge clk)` 與 `default disable iff (!rst_n)`，因此為求聚焦，省略時脈與重置。
...
> **設計意圖。** 一個*終將*被回應的請求是活性（liveness）概念；給它一個真實的期限（`##[1:N]`），就把它變成模擬實際能讓它失敗的安全性檢查。請以協定的最差延遲為視窗界限。
...
「保持到被接受」正是讓接收方能依*自己*的步調取樣請求與其酬載的條件。少了它，發送方就可能在交易中途改動資料。
...
> **設計意圖。** 這是 ready/valid stream 的核心契約：生產者在消費者取走前，不得撤回或變動已提供的一拍。一條斷言便編碼了整個握手的穩定性規則。

## One-hot 與 one-hot-zero 狀態

one-hot 訊號恰好帶有一個設定位元；one-hot-zero 則額外允許全零。這些是不變式，每個週期檢查，無需前提（antecedent）：
...
FIFO 有三個經典的安全性性質。各以「絕不能發生」的形式陳述。
...
> **設計意圖。** 溢位與下溢會無聲地破壞資料，且在波形上難以察覺。化為斷言後，它們會在規則被打破的確切週期成為響亮、即時的失敗——這是 FIFO 上最有價值的單一檢查。

若推入與彈出在已滿或已空的 FIFO 上同時發生，count 必須維持在範圍內；界限 `count <= DEPTH` 加上各操作的防護，便涵蓋了這些角落情形。

## Gray-code 單一位元變化

gray 編碼的計數器（常是跨時脈域的 FIFO 指標）每步必須恰好改變一個位元。此檢查使用 `$past` 與 `$countones`：
...
> **設計意圖。** 將指標 gray 編碼的全部理由，就在於單一位元的變化能安全地跨時脈域取樣。這條斷言驗證了同步器所依賴的性質；多位元的跳變代表編碼器壞了，CDC 不安全。
...
某些控制訊號必須是單一週期的脈衝（pulse），而非持續的位準（level）。斷言該訊號在下一週期回到低態：
...
跨時脈域（clock-domain-crossing）的正確性，大多*並非*單時脈的 SVA 性質。跨越邊界的資料，依定義相對於接收時脈是不穩定的，因此對它做天真的同時脈斷言不是無意義就是錯誤。SVA *能*陳述的，是 CDC 結構所依賴的意圖：
...
至於跨越本身——同步器深度、穩定時間、亞穩態線網上不得有組合邏輯——請使用專門的 CDC 工具，或在 `##` 邊界流動的多時脈斷言（第 8 章）。將結構性 CDC 簽核視為獨立的工作項；以 SVA 釘住該結構所假設的*編碼*不變式。

## 常見陷阱

- **模擬中的無界 handshake 檢查。** `req |-> s_eventually ack` 在有限時間內無法失敗。請使用有界視窗 `##[1:N]`。
- **忘記酬載穩定性。** 只檢查 `valid` 維持，卻不檢查 `data` 維持，會讓生產者變動已提供的一拍。請加上 `$stable(data)`。
- **在閒置合法處使用 `$onehot`。** 閒置時全零的 grant 匯流排需要 `$onehot0`，而非 `$onehot`。
- **對 CDC 線網做同時脈斷言。** 對不穩定訊號而言是無意義的。請檢查來源域的編碼不變式，並將穩定交給 CDC 流程。
- **混淆 pulse 與 level 意圖。** 決定協定要的是單一週期的脈衝還是持續的位準，並準確地斷言之。
...
- FIFO 安全性是三個「絕不」性質：不溢位、不下溢、count 在範圍內。
