...
[← 斷言種類](03-assertion-kinds.md) · [目錄](../README.md) · [下一章：序列：基礎 →](05-sequences-basics.md)

## 學習目標

- 撰寫對取樣值（sampled value）運算的布林運算式。
...
SVA 以分層建構。最底層是**布林層**（boolean layer）：在單一取樣點評估為真或假的一般運算式。序列（第 5 章）與性質（第 7 章）建構於這些布林之上。把布林層做對，是其上一切的基礎。

並行斷言中的布林運算式使用與 RTL 相同的運算子——`&`、`|`、`==`、`<`、`&&` 等——但它運算的是**取樣值**（第 2 章），而非活值。因此斷言中的 `a && b` 意指「取樣的 `a` 與取樣的 `b`，於此時脈緣之前在 Preponed 取得」。
...
布林常需把本週期的值與前一週期的值相比。取樣值函式正是如此運作，全部相對於斷言時脈。

### $rose、$fell、$stable、$changed

這些函式把現在的取樣值與一個時脈之前的取樣值相比：

- **`$rose(e)`**——當 `e` 自上一週期由 0 變為 1 時為真（取樣值的上升緣）。
...
`$rose` 與 `$fell` 測試單一位元（向量取最低有效位）。`$stable` 與 `$changed` 對整個運算式運作，包含向量。由於它們相對於前一個*取樣*值比較，它們是斷言層談論邊緣的方式——不要試圖用對手動延遲副本的原始 `==` 比較來重建邊緣。

### $past

`$past(e)` 回傳 `e` 在若干週期前所持有的取樣值：
...
`$past` 亦接受選用的閘控運算式與時脈；日常使用中，單引數與雙引數形式已能涵蓋多數需求。注意：在模擬早期、尚未經過 `n` 個週期之前，`$past` 回傳重置／初始值——請設計斷言使此啟動區間不產生誤報（第 8 章的 `disable iff` 與蘊涵的空真皆有助於此）。
...
`$isunknown(e)` 在 `e` 的任一位元為 `x` 或 `z` 時為真。它是斷言層對未知值抵達不該抵達之處的防護：
...
這是廉價而高價值的檢查。模擬中溜過去的未知值，往往在矽中成為真正的錯誤；對關鍵訊號斷言 `!$isunknown` 可在源頭捕捉它們。

> **設計意圖。** 布林層是設計者陳述「必須在單一邊緣成立」之事實之處：這個向量是 one-hot、
> 這個控制字永不未知、這個暫存器擷取了正確的值。這些是你對每個訊號所懷抱的不變式——
> 取樣值函式讓你以斷言時脈的語彙把它們寫下來。

## 常見陷阱

- **忘記布林使用取樣值。** 斷言中的 `a && b` 是取樣的 `a` 與 `b`，而非其活值。當某 `always` 區塊本步正在更新它們時，這一點很重要。
- **手工撰寫邊緣偵測。** 請用 `$rose`／`$fell`，而非與手動延遲副本相比；取樣值版本就構造而言是正確的。
- **忽略 `$past` 在啟動時的行為。** 在足夠的週期經過之前，`$past` 回傳初始值。以 `disable iff (reset)` 閘控此類斷言，或倚賴蘊涵的空真，使啟動區間不觸發。
