...
在寫跨週期規則以前，先要能可靠地說出「這一拍」發生了什麼。`req` 是不是剛上升？`data` 在 `valid` 時有沒有保持？`state` 是否仍然 one-hot？如果這些單週期判斷本身含糊，後面再漂亮的 sequence 或 property 都只是把含糊放大。

因此本章先停在最底層：如何用 sampled value 寫出穩定的布林事實。等這些小判斷可信了，下一章才把它們串成時間上的形狀。
