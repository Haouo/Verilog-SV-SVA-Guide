...
- 認識標準的可重用 assertionlibrary：OVL 與 Accellera SVA library。
- 為跨專案的重用而封裝你自己的 assertion。

## 設計者 mental model

checker 是帶有 named interface 的 reusable assertion container。它讓 team 把 protocol rule package
一次，然後在同樣 rule 出現的地方 instantiate。library 於是變成 design intent 的 vocabulary：no
overflow、stable until accept、one-hot、latency bounded 等等。

checker 有用的 interface 應該小但精準。它 expose state rule 所需的 signal、parameter、clocking、
reset，同時 hide caller 不應重寫的 SVA detail。reuse 只有在 checker name 和 parameter 讓 intent
一眼明白時才有價值。
...
- **交付不含其 cover 的 checker。** 被重用的 assertion 在新場址可能 vacuous pass。請納入 antecedentcover，使檢查自守其 vacuity。
...
- 將專案特定的意圖封裝成被 bind、參數化、且自帶 antecedentcover 的 checker。
